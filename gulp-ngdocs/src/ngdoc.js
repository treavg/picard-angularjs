...
Doc.prototype = {
  ...
  parse: function() {
    ...
    function flush() {
      if (atName) {
        ...
        } else if (atName == 'raises' || atName == 'raise') {
          match = text.match(/^\{([^}]+)\}\s+(([^\s=]+)|\[(\S+)=([^\]]+)\])\s+(.*)/);
                             //  1      1    23       3   4   4 5      5  2   6  6
          if (!match) {
            throw new Error("Not a valid 'raises' format: " + text + ' (found in: ' + self.file + ':' + self.line + ')');
          }
  ...
  html_usage_parameters: function(dom) {
    ...
    if(params.length > 0) {
      ...
      // console.log(params);
      dom.html('<h2>Function Argument Object Parameters</h2>');
  ...
  html_usage_raises: function(dom) {
    var self = this;

    var errors = this.error ? this.error : [];
    if(errors.length > 0) {
      dom.html('<h2>Error Messages</h2>');
      dom.html('<table class="variables-matrix table table-bordered table-striped">');
      dom.html('<thead>');
      dom.html('<tr>');
      dom.html('<th>Type</th>');
      dom.html('<th>Status Code</th>');
      ...
      function processErrors(errors) {
        for(var i=0;i<errors.length;i++) {
          ...
          for(var j=0;j<types.length;j++) {
            var type = types[j];
            dom.html('<a href="" class="number">');
            dom.text(type);
            dom.html('</a>');
          }
          dom.html('</td>');
          dom.html('<td>');
          dom.html(error.description);
          dom.html('</td>');
          dom.html('</tr>');
        };
      }
  ...
  //   // console.log("\n\n\n html raises  \n\n\n")
  //   if (self.raises) {
  //     dom.html('<h2>Raises</h2>');
  //     dom.html('<table class="variables-matrix">');
  //     dom.html('<tr>');
  //     dom.html('<td>');
  //     dom.html('<a href="" class="number">');
  //     dom.text(self.raises.type);
  //     dom.html('</a>');
  //     dom.html('</td>');
  //     dom.html('<td>');
  //     dom.html(self.raises.description);
  //     dom.html('</td>');
  //     dom.html('</tr>');
  //     dom.html('</table>');
  //   }
    ...
    if (self.methods.length) {
      dom.div({class:'member method'}, function(){
        dom.h('Methods', self.methods, function(method){
          ...
          // if method is not in picard internal then show its actual function params.
          methodOf = method.methodOf.split(".")[2]
          console.log(methodOf);
          methodParams = '(object)';
          if(methodOf == 'picard'){
            methodParams = '(' + signature.join(', ') + ')'
          }
          dom.h(method.shortName + methodParams, method, function() {
            dom.html(method.description);
            method.html_usage_parameters(dom);
            self.html_usage_this(dom);
            method.html_usage_returns(dom);
            method.html_usage_raises(dom);
